...
bool buzzerSilencedLocal = false;
...
void onWsEvent(AsyncWebSocket *server, AsyncWebSocketClient *client,
  ...
  if (type == WS_EVT_CONNECT) {
    ...
    if (!err && cmdDoc["com"] == "off_buzzer") {
      // Silence uniquement local
      currentAlert = false;
      updateActuators(false);
      ws.textAll("{\"event\":\"off_buzzer\",\"state\":true,\"scope\":\"local\"}");
      Serial.println("[Récepteur] Buzzer mis en silence local");
      return;  // ← très important
    }

    // Toutes les autres commandes → on les transmet à l'émetteur
...
void updateActuators(bool alert) {
  ...
  // Buzzer : on respecte le silence local
  bool buzz = alert && !buzzerSilencedLocal;
